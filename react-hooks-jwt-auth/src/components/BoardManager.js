...
const BoardManagement = ({ setGeneratedReportProp }) => {
  ...
  const fetchLifecycleEvents = () => {
    ManagerService.viewAllLifecycleEvents()
      .then(response => {
  ...
  const generateReport = async () => {
    try {
      ...
      const reportResponse = await ManagerService.generateLifecycleReports(selectedAssets);
  ...
  const handleCheckboxChange = (relatedId) => {
    // Toggle selection of the asset
    if (selectedAssets.includes(relatedId)) {
      setSelectedAssets(selectedAssets.filter(id => id !== relatedId));
    } else {
      setSelectedAssets([...selectedAssets, relatedId]);
    }
  };
          ...
          {lifecycleEvents.map(event => (
            <tr key={event.eventId}>
              <td>
                <input
                  type="checkbox"
                  checked={selectedAssets.includes(event.relatedId)}
                  onChange={() => handleCheckboxChange(event.relatedId)}
                />
              </td>
              <td>{event.eventId}</td>
          ...
          {generatedReport.map(report => (
            <div key={report.eventId} className="card mb-3">
              <div className="card-body">
                ...
                <p className="card-text">Related ID: {report.relatedId}</p>
